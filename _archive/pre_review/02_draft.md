...
먼저 이 모델이 어떻게 만들어졌는지부터 보자. 기존에 나와 있던 AI 성숙도 시도를 100건 넘게 훑었고, 그중 서른 개 남짓은 정밀하게 뜯어봤다. 임원 스무 명 안팎을 인터뷰하고, 실무자 약 600명에게 설문을 돌렸다. 그리고 포춘 500대 기업 여러 곳에서 실제로 파일럿을 굴려 검증했다.[^c2n2]
...
인정하지 않고 시작하면 뒤에 할 이야기가 전부 헐거워진다. 그러니 이 장은 "그들이 부족하다"는 말로는 한 줄도 나아가지 않는다.
...
세어놓고 숫자를 한참 들여다봤다. 가운데 두 단계에만 스물하나, **전체의 84퍼센트가 몰려 있다.** 위의 두 단계에는 각각 하나씩만 있다. 다섯 단계짜리 모델인데 무게는 아래쪽 절반에 다 실려 있는 셈이다.
...
**나는 반대로 놓았다.** 이 책이 만드는 자에서 검증은 4단계, 성숙의 분수령에 있다.

왜 그렇게 놓았는지는 1장에서 이미 반쯤 답했다. 항목 단위로 대봤을 때 비어 있던 자리가 어디였는지 떠올려보자. 만드는 데서 막힌 게 아니었다. **만든 다음에서 막혀 있었다.** 어디에 올릴지, 무엇으로 붙일지, 누가 이어받을지, 잘 돌아가는지 무엇으로 확인할지. 배우고 만드는 앞쪽은 그럭저럭 채워져 있는데 뒤쪽이 통째로 비어 있는 그림이 반복해서 나온다.

그러니 이렇게 정리하자. **SEI는 기초를 갖추는 일이 어렵다고 보고, 우리는 기초를 갖춘 다음이 진짜 문제라고 본다.** 그들이 틀렸다는 말이 아니다. 보는 자리가 다르다. 그리고 어느 쪽이 맞는지는 각자 자기 조직을 대봐야 안다. 앞쪽이 안 채워진 조직이라면 SEI의 배치가 맞는 진단을 줄 것이다. 만들 줄은 알게 됐는데 그다음이 안 굴러가는 조직이라면 이야기가 달라진다.
...
두 장면 다 성숙도의 문제이고, 팀장이 가장 먼저 부딪히는 벽이다. 그런데 세계에서 가장 정교한 자에는 그걸 잴 눈금이 없다. 자기 조직에서 제일 아픈 데를 재려고 자를 들었는데 거기에 눈금이 없는 걸 알았을 때, 그 기분은 좀 서늘하다. 그래서 이 책이 만드는 자에는 **자원·비용을 독립된 축으로 세웠고**, 담당자 인계를 문항으로 직접 묻는다.
...
**셋.** 차이는 무게중심이다. 그 모델은 역량영역의 84퍼센트를 아래 두 단계에 놓았고, 검증조차 2단계, 즉 배포하려면 당연한 기초로 본다. 우리 자는 검증을 4단계, 성숙의 분수령에 놓는다. 만들 줄 알게 된 다음에서 막히는 조직에는 이쪽 배치가 더 맞다.

**넷.** 그리고 그 모델에는 예산·자원을 재는 축이 없고 담당자 인계를 묻는 문항이 없다. 포춘 500대 기업 표본에서는 문제가 아니기 때문이다. 우리에게는 대개 가장 먼저 막히는 두 가지다.

정리하면 이렇다. 남이 만든 좋은 자를 가져다 쓰지 않는 이유는 그 자가 나빠서가 아니라, **그 자가 우리 조직에서 제일 아픈 데를 재도록 설계되지 않았기 때문**이다. 다음 보고에서 성숙도 모델 도입을 검토한다면 질문을 하나만 얹어보자. "이 자로 우리가 지금 제일 답답한 걸 잴 수 있는가."
