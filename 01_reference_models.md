...
> **저자가 짚어야 할 대목:** CMMI는 "개선 도구"로 태어났지만 현장에서는 **"등급 취득"이 목적이 되는 순간 망가졌다.** 심사를 통과하려고 문서를 만들고, 등급을 딴 뒤 원래대로 돌아가는 일이 흔했다. 20년간 그걸 지켜본 사람이 AX 성숙도 모델을 만든다면, 그 실패를 되풀이하지 않게 설계해야 한다. → **이것이 "심사 없는 셀프 점검"이라는 결정의 진짜 이유다.** 등급을 남에게 증명하는 순간 게임이 시작되기 때문이다.
...
성숙도 모델은 오래전부터 비판받아 왔고, 저자는 이걸 **회피하지 말고 책 앞부분에서 정면으로 다뤄야 한다.** 그래야 20년 경력자가 쓰는 책이 된다.
